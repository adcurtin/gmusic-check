...
def aa_search(search_string, max_results):
    return api.search(search_string, max_results=max_results).get('song_hits')
...
def update_stats(track, stats):
    ...
    if track.get('genre'):
        stats['genres'].append(track.get('genre'))
    if track.get('year'):
        stats['years'].append(track.get('year'))
    if track.get('playCount'):
        stats['total_playcount'] += track.get('playCount')
